...
class Indexuser extends Backend
{
	...
	public function index(){
		$result = Db::name('indexuser')->order("`userid` desc")->select();
	...
	if($this->request->isPost()){
       ...
       $data=array(
          ...
          // }
		  // }
		  $this->redirect('indexuser/index');
		  }
		$id = $this->request->param('id');
		$result = Db::name('indexuser')->where("`userid`=$id")->find();
		  //sprint ($result);
		  //die;
		$this->assign('result',$result);
	...
	public function edituser(){
       ...
       $data=array(
		  ...
		  if(!empty($photo)){	  
			  Db::name('indexuser')->where("`userid`=$id")->update(['photo'=>$photo]);
		  }
		  if(!empty($_POST['password'])){
			  $dd =md5($_POST['password']);
			  Db::name('indexuser')->where("`userid`=$id")->update(['password'=>$dd]);
		  }
		  // echo Db::name('role_user')->getLastSql();
		  // die();
		  $result = Db::name('indexuser')->where("`userid`=$id")->update($data);
		  $this->redirect('indexuser/index');
		  // if(isset($_POST['items'])){
		  // $pkvs=$_POST['items'];
	...
	public function deluser(){
		$id = $this->request->param('id');
	    Db::name('indexuser')->where("`userid`=$id")->delete();
		// Db::name('role_user')->where("`user_id`=$id")->delete();
		$this->success("操作成功", url('indexuser/index', array(), 'html'));
	}
}