...
class Cases extends Backend
{
    ...
    public function index(){
        ...
        $result = Db::name('case_category')->order("`catid` desc")
		->alias('c')
	...
	if($this->request->param('id') != 'undifined'){
	 ...
	 $num = Db::name('case_category')->where('parent_id',$id)->count();
     $type = true;
	 if($num){
		 $type = false;
	     }
	 if(!$type){
      ...
      $result = Db::name('case_category')->where("`catid` = $id")->delete();
      $this->success("操作成功", url('cases/index', array(), 'html'));
	 }
	}
	}
    ...
    if ($pkvs) {
			...
			foreach($pkvs as $value){
				 $num = Db::name('case_category')->where('parent_id',$value)->count();
	...
	if($num){
	 ...
	 if(!$type){
		 $this->error("有子分类不能删除");
            ...
            $rs = Db::name('case_category')->where("`catid` in ($ids)")->delete();
           // echo Db::name('case_category')->getLastSql();
			$this->success("批量删除成功",url('cases/index', array(), 'html'));
        }
    }
}
}
    ...
    public function addcategory(){
		 if($this->request->isPost()){
		  ...
		  //sprint ($mm);
		  //die;
		  Db::name('case_category')->insert($data);
		  //$this->redirect('cases/category');
		  $this->redirect('cases/index',array('id' => $mm));
		 }
		  $catid = (int)$this->request->param('id');
		  //echo $catid;//打印出子分类的parent_id
		  //die;
		  $catData = Db::name('case_category')->where('catid', $catid)->find();//找到一条数据，它的catid是子类的parent_id
	...
	public function editcategory(){
        ...
        if($this->request->isPost()){
            ...
            Db::table('hr_case_category')->where(array('catid'=>$id))->update($data1);
            $this->redirect('cases/index',array('id' =>$mm));
           }else{   
           //展示需要修改的数据
                    $id=$this->request->param('id');
                    ...
                    $result=Db::name('case_category')->where(array('catid' => $id))->select();
                    $this->assign('result',$result);
					$catid = (int)$this->request->param('id');
                    // sprint ($catid);
					// die();					
		            $catData = Db::name('case_category')->where('catid', $catid)->find();
					// sprint ($catData);
					// die();
		            $this->assign('catData',$catData);//分配到前台
                    return $this->fetch();
