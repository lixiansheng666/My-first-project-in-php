...
class Shopuser extends Backend
{
	...
	public function index(){
		$result = Db::name('shop_user')->order("`userid` desc")->select();
	...
	if($this->request->isPost()){
       ...
       $data=array(
		  ...
		  $result = Db::name('shop_user')->insert($data);
		...
		$result = Db::name('shop_user')->where("`userid`=$id")->find();
		  //sprint ($result);
		  //die;
		$this->assign('result',$result);
	...
	public function edituser(){
       ...
       $data=array(
		  ...
		  if(!empty($photo)){	  
			  Db::name('shop_user')->where("`userid`=$id")->update(['photo'=>$photo]);
		  }
		  if(!empty($_POST['password'])){
			  $dd =md5($_POST['password']);
			  Db::name('shop_user')->where("`userid`=$id")->update(['password'=>$dd]);
		  }
		  // echo Db::name('role_user')->getLastSql();
		  // die();
		  $result = Db::name('shop_user')->where("`userid`=$id")->update($data);
		  $this->redirect('shopuser/index');
		  // if(isset($_POST['items'])){
		  // $pkvs=$_POST['items'];
	...
	public function deluser(){
		$id = $this->request->param('id');
	    Db::name('shop_user')->where("`userid`=$id")->delete();
		// Db::name('role_user')->where("`user_id`=$id")->delete();
		$this->success("操作成功", url('shopuser/index', array(), 'html'));
	}
}